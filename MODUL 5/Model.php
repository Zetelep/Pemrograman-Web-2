...
{
    $sql = "INSERT INTO `peminjaman` (`tgl_pinjam`, `tgl_kembali`, `id_member`, `id_buku`) VALUES (:tglpinjam,:tglkembali,:id_member,:id_buku)";
    $stmt = koneksi()->prepare($sql);
    $result = $stmt->execute(array(':tglpinjam' => $tglpinjam, ':tglkembali'=> $tglkembali, ':id_member' => $id_member, ':id_buku' => $id_buku));
...
{
    $pdo_statement = koneksi()->prepare(
        "UPDATE peminjaman SET tgl_pinjam='" . $tglpinjam ."', tgl_kembali='" . $tglkembali . "', id_buku='".$id_buku."', id_member='".$id_member."' WHERE id_peminjaman = ". $id
    );
